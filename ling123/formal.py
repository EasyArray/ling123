...
def chunk_pattern(p):
  ...
  return tuple(y   for x in p.split('}') if x
              for y in x.split('{')       )
...
def get_outputs(s, rules):
  ...
  return [(found.add(o) if o not in found else False) or o for ip, op in rules for o in run_rule(ip,op,s)]

def theorem_tree(s, rules, maxdepth=5, reset_found = True):
  global found
  if reset_found: found = set()
